...
public class DataSourceConfig implements WebMvcConfigurer {
    ...
    @Value("${spring.datasource.url:jdbc:mysql://localhost:3306/trackify_master?useSSL=false&allowPublicKeyRetrieval=true}")
    ...
    private synchronized void initializeTenantDataSource(Long tenantId, 
        ...
        try {
            Map<String, Object> tenant = masterJdbcTemplate.queryForMap(
            ...
            String url = String.format("jdbc:mysql://%s:%d/%s?useSSL=false&allowPublicKeyRetrieval=true", host, port, dbName);
