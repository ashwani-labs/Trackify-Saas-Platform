...
public class ProjectController {
  ...
  public ResponseEntity<ApiResponse<ProjectStatsResponse>> getProjectStats(
    ...
    return ResponseEntity.ok(
        ApiResponse.ok("Stats fetched successfully", projectService.getProjectStats(userId, role)));
  }
  ...
  public ResponseEntity<ApiResponse<ProjectResponse>> getProjectById(
      @RequestHeader("Authorization") String authHeader, @PathVariable("id") Long id) {
    String token = authHeader.substring(7);
    Long userId = jwtUtil.extractUserId(token);
    String role = jwtUtil.extractRole(token);
